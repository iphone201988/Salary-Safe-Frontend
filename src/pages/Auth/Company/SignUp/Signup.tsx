...
import { ChangeEvent, FormEvent, useState } from "react";
import InputField from "../../../../components/InputField/InputField";
import {  companyRegistrationStep1Schema, companyRegistrationStep2Schema, companyRegistrationStep3Schema, validateForm } from "../../../../Schema/Schemas";
import Button from "../../../../components/Button/Button";
import { toast } from "react-toastify";
import axios from "axios";
import { companyRegister } from "../../../../API/apis";
...
const [isSubmitting, setIsSubmitting] = useState(false);
  ...
  const ProgressBar = () => (
    <div className="w-full bg-gray-200 rounded-full h-2.5 mb-5">
      <div className={`bg-[#019529] h-2.5 rounded-full`} style={{ width: `${(step / 3) * 100}%` }}></div>
    </div>
  );
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-between mt-6">
            ...
            {step < 3 && (
              ...
              <Button
                content={isSubmitting ? "Creating Account..." : "Create Account"}
                type="submit"
                className="bg-[#019529] text-white px-4 py-2 rounded-md"
              />
            )}
